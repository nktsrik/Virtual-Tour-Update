...
export default function PengelolaDashboard() {
  const [stats, setStats] = useState({ totalTours: 0, totalGaleri: 0, totalLokasi: 0, needsRevision: 0 });
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setActivities(acts);
      setStats({ totalTours: tours.length, totalGaleri: galeri.length, totalLokasi: lokasi.length, needsRevision: tours.filter(t => t.status_id === 5).length });
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const statCards = [
    ...
    { label: 'Perlu Revisi', value: stats.needsRevision, sub: 'Diminta perbaikan', highlight: stats.needsRevision > 0 },
  ];

  const quickActions = [
    { label: 'Tambah Virtual Tour', sub: 'Upload panorama 360°', href: '/pengelola/virtual?action=tambah', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10l4.553-2.069A1 1 0 0121 8.87v6.26a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z" /> },
    { label: 'Tambah Galeri', sub: 'Upload foto & media', href: '/pengelola/galeri', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /> },
    { label: 'Tambah Lokasi', sub: 'Tambah titik lokasi baru', href: '/pengelola/lokasi?action=tambah', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z" /> },
    { label: 'Virtual Tour Saya', sub: 'Kelola semua tour', href: '/pengelola/virtual', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /> },
    { label: 'Perlu Revisi', sub: `${stats.needsRevision} tour perlu diperbaiki`, href: '/pengelola/virtual', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />, badge: stats.needsRevision > 0 },
    ...
    <div className="p-6 lg:p-8 min-h-screen bg-[#f9fafb]">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-2xl font-bold text-gray-900 mt-0.5">Dashboard Pengelola</p>
        </div>
      ...
      {stats.needsRevision > 0 && (
        <div className="bg-white border border-orange-200 rounded-xl p-4 mb-6 flex items-center gap-4">
          <div className="w-2 h-10 bg-orange-400 rounded-full flex-shrink-0" />
          <div className="flex-1">
            <p className="text-sm font-medium text-gray-800">{stats.needsRevision} virtual tour perlu direvisi</p>
            <p className="text-xs text-gray-400 mt-0.5">Admin meminta perbaikan pada konten Anda</p>
          </div>
          <Link href="/pengelola/virtual" className="text-sm font-medium text-[#16341B] hover:underline flex-shrink-0">
            Lihat →
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statCards.map((s) => (
          <div key={s.label} className={`bg-white rounded-xl p-5 border ${s.highlight ? 'border-orange-200' : 'border-gray-100'}`}>
            <p className="text-xs text-gray-400 mb-1">{s.label}</p>
            <p className={`text-3xl font-bold ${s.highlight ? 'text-orange-600' : 'text-gray-900'}`}>{s.value}</p>
