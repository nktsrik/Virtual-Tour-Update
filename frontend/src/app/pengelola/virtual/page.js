...
export default function PengelolaVirtualTourPage() {
  ...
  useEffect(() => {
    if (searchParams.get('action') === 'tambah') {
      router.push('/pengelola/virtual/tambah');
    }
    loadTours();
    loadLokasi();
  }, []);
  ...
  const loadLokasi = async () => {
    try {
      ...
      Swal.fire({
        ...
        text: error.response?.data?.message || 'Terjadi kesalahan saat memuat data lokasi'
      });
    }
  };
  ...
  const loadTours = async () => {
    try {
      ...
      console.log('Tours data loaded:', toursData); // Debug log
      console.log('Sample tour with rejection:', toursData.find(t => t.status_id === 4)); // Debug rejected tour
      setTours(toursData);
    } catch (error) {
      console.error('Error loading tours:', error);
      Swal.fire({
        icon: 'error',
        title: 'Gagal Memuat Virtual Tour',
        text: error.response?.data?.message || 'Terjadi kesalahan saat memuat data virtual tour'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('💾 Submitting tour...', { editingTour, formData });
      
      if (editingTour) {
        console.log('📝 Updating tour ID:', editingTour.id);
        const response = await virtualTourService.updateTour(editingTour.id, formData);
        console.log('✅ Update response:', response);
        
        await Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: response.message || 'Virtual tour berhasil diupdate',
          confirmButtonColor: '#16a34a'
        });
      } else {
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: response.message || 'Virtual tour berhasil ditambahkan. Status: Draft',
          confirmButtonColor: '#16a34a'
        });
      }
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Gagal!',
        text: error.response?.data?.message || error.message || 'Gagal menyimpan virtual tour',
        confirmButtonColor: '#dc2626'
      });
    }
  };
  ...
  const handleEdit = (tour) => {
    router.push(`/pengelola/virtual/edit?id=${tour.id}`);
  };
  ...
  const handleSubmitForReview = async (id) => {
    const result = await Swal.fire({
      ...
      text: 'Virtual tour akan dikirim ke admin untuk review dan approval',
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#16a34a',
      cancelButtonColor: '#6b7280',
    ...
    if (result.isConfirmed) {
      try {
        await virtualTourService.submitForReview(id);
        await Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: 'Virtual tour berhasil disubmit untuk review',
          confirmButtonColor: '#16a34a'
        });
        loadTours();
      } catch (error) {
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Gagal!',
          text: error.response?.data?.message || 'Gagal submit virtual tour',
          confirmButtonColor: '#dc2626'
        });
      }
    }
  ...
  const getStatusBadge = (statusId) => {
    const statusMap = {
      ...
      5: { text: 'Perlu Perbaikan', color: 'bg-amber-100 text-amber-800 border-amber-300' },
    };
  ...
  const filteredTours = tours.filter(tour => {
    ...
    if (filter === 'draft') return tour.status_id === 1;
    ...
    if (filter === 'revision') return tour.status_id === 5;
    return true;
  });

  const stats = {
    total: tours.length,
    draft: tours.filter(t => t.status_id === 1).length,
    ...
    revision: tours.filter(t => t.status_id === 5).length,
  };
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Buat dan kelola virtual tour Anda</p>
          </div>
          <button
            onClick={() => router.push('/pengelola/virtual/tambah')}
          ...
          <div className="flex items-start">
            ...
            <div>
              <p className="text-yellow-800">
                ⚠️ Belum ada lokasi tersedia. Hubungi admin untuk menambahkan lokasi.
              </p>
            </div>
          </div>
        </div>
                    ...
                    {lokasi.length === 0 ? (
                      <div className="mt-2 p-3 bg-red-50 border-l-4 border-red-400 rounded-r-lg">
                        <p className="text-xs text-red-700 flex items-center">
                          ...
                          Belum ada lokasi tersedia. Hubungi admin.
                        </p>
                      </div>
                    ) : formData.lokasi_id && (
                  ...
                  <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded-r-lg">
                    <div className="flex items-start">
                      ...
                      <div className="text-sm text-red-800">
                        ...
                        <p>Belum ada lokasi tersedia. Silakan hubungi administrator untuk menambahkan lokasi terlebih dahulu sebelum membuat virtual tour.</p>
                      </div>
                    </div>
                  </div>
                )}
      ...
      <div className="bg-white rounded-xl shadow-md mb-6 p-2">
        <div className="flex gap-2">
          {[
            ...
            { key: 'revision', label: 'Perlu Perbaikan', count: stats.revision },
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        {filteredTours.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-sm">Klik tombol &quot;Tambah Virtual Tour&quot; untuk membuat yang baru</p>
          </div>
        ) : (
          <>
                    ...
                    <tr key={tour.id} className="hover:bg-green-50/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {tour.image_path ? (
                            <img
                              src={`http://localhost:5001${tour.image_path}`}
                              alt={tour.nama}
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-0.5 line-clamp-1 max-w-xs">
                              {tour.deskripsi || tour.description || 'Tidak ada deskripsi'}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        {tour.lokasi_nama ? (
                          <span className="inline-flex items-center px-2.5 py-1 text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full font-medium">
                            {tour.lokasi_nama}
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5 flex-wrap">
                          {tour.status_id === 1 && (
                            <>
                              <button onClick={() => handleEdit(tour)} className="px-2.5 py-1 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-md transition-colors">Edit</button>
                              <button onClick={() => handleSubmitForReview(tour.id)} className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-md transition-colors">Submit</button>
                              <button onClick={() => handleDelete(tour.id)} className="px-2.5 py-1 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-md transition-colors">Hapus</button>
                            </>
                          )}
                          {tour.status_id === 2 && (
                          ...
                          {tour.status_id === 3 && (
                            <span className="px-2.5 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-md">Dipublikasikan</span>
                          )}
                          {tour.status_id === 4 && (
                            <>
                              {tour.alasan_penolakan && (
                                <button onClick={() => Swal.fire({
                                  title: 'Alasan Penolakan',
                                  html: `<div style="text-align:left"><p style="margin-bottom:8px"><strong>${tour.nama || tour.title}</strong></p><div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px">${tour.alasan_penolakan}</div></div>`,
                                  icon: 'error', confirmButtonColor: '#dc2626',
                                })} className="px-2.5 py-1 bg-red-100 hover:bg-red-200 text-red-700 text-xs font-medium rounded-md transition-colors">Lihat Alasan</button>
                              )}
                              <button onClick={() => handleDelete(tour.id)} className="px-2.5 py-1 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-md transition-colors">Hapus</button>
                            </>
                          )}
                          {tour.status_id === 5 && (
                            <>
                              <button onClick={() => Swal.fire({
                                ...
                                html: `<div style="text-align:left"><p style="margin-bottom:8px"><strong>${tour.nama || tour.title}</strong></p><div style="background:#fef9c3;border:1px solid #fde047;border-radius:8px;padding:12px">${tour.alasan_penolakan || 'Tidak ada catatan'}</div></div>`,
                                icon: 'info', confirmButtonColor: '#16a34a',
                              })} className="px-2.5 py-1 bg-amber-100 hover:bg-amber-200 text-amber-700 text-xs font-medium rounded-md transition-colors">Catatan</button>
                              <button onClick={() => handleEdit(tour)} className="px-2.5 py-1 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-md transition-colors">Perbaiki</button>
                              <button onClick={() => handleSubmitForReview(tour.id)} className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-md transition-colors">Submit Ulang</button>
                              <button onClick={() => handleDelete(tour.id)} className="px-2.5 py-1 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-md transition-colors">Hapus</button>
                            </>
                          )}
                        </div>
      ...
      {stats.draft > 0 && (
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-blue-800 font-medium">ℹ️ Anda memiliki <strong>{stats.draft}</strong> virtual tour dalam status Draft. Klik &quot;Submit Review&quot; untuk mengirimkan ke admin.</p>
        </div>
      )}
      ...
      {stats.revision > 0 && (
        <div className="mt-6 bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="text-amber-800 font-medium">✏️ Anda memiliki <strong>{stats.revision}</strong> virtual tour yang perlu diperbaiki. Lihat catatan dari admin, lakukan perbaikan, lalu submit ulang.</p>
        </div>
      )}
    </div>
  );
