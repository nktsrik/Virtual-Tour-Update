...
function EditLokasiAdminForm() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const lokasiRes = await fetch(`${API_URL}/api/lokasi/${id}`);
        ...
        if (lokasiData.success) {
          ...
          if (lokasi.thumbnail) {
            setThumbnailPreview(lokasi.thumbnail.startsWith('http') ? lokasi.thumbnail : `${API_URL}${lokasi.thumbnail}`);
          }
        }
        const kategoriRes = await fetch(`${API_URL}/api/lokasi/kategori/list`);
          ...
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex justify-between mt-1">
                ...
                <p className="text-xs text-gray-500"><span className={form.deskripsi.length > 200 ? 'text-orange-500 font-semibold' : ''}>{form.deskripsi.length}</span>/250</p>
              </div>
            </div>
            ...
            <div>
              <MapPicker value={{ x: form.map_x, y: form.map_y }} onChange={({ x, y }) => setForm(f => ({ ...f, map_x: x, map_y: y }))} />
            </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-gray-200">
              ...
              <button type="submit" disabled={submitting || kategoriList.length === 0}
                className="px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed font-medium flex items-center space-x-2">
                {submitting ? (
                  <><svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg><span>Memperbarui...</span></>
                ) : (
                  <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg><span>Perbarui Lokasi</span></>
                )}
              </button>
            </div>
          </form>
...
export default function AdminEditLokasi() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center">
        <div className="text-center">
